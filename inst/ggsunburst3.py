...
from ete3.parser.newick import *
from ete3.coretype.tree import *
...
#        t._labels(xlim=xlim, rot=rot, topology_only=False)
    ...
    node_attributes_str = [str(x) for x in node_attributes]
    rects   = ["xmin","xmax","ymin","ymax","x","name","leaf"]
    # rect_str = '\t'.join(rects + map(str, node_attributes)) + '\n'
    rect_str = '\t'.join(rects + node_attributes_str) + '\n'

    node_labels  = ["x","y","label","rangle","rhjust","pangle","pvjust","delta_angle","xfraction"]
    node_labels_str = '\t'.join(node_labels + node_attributes_str) + '\n'

    leaf_labels  = ["x","y","y_out","label","rangle","pangle","rhjust"]
    leaf_labels_str = '\t'.join(leaf_labels + node_attributes_str).replace('rangle','angle').replace('rhjust','hjust') + '\n'
